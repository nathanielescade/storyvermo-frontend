...
const ShareModal = ({ isOpen, onClose, shareData, imageUrl, isVerse }) => {
  ...
  const handleSocialShare = (platform) => {
    ...
    switch (platform) {
      ...
      case 'youtube':
        ...
        handleCopyLink();
        alert('Link copied! You can now paste it in your YouTube video description.');
        return;
      case 'xing':
        shareUrl = `https://www.xing.com/spi/shares/new?url=${encodedUrl}`;
        break;
      default:
        return;
    }
  ...
  const socialPlatforms = [
    ...
    { name: 'Discord', key: 'discord', color: '#7289DA', svg: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    { name: 'YouTube', key: 'youtube', color: '#FF0000', svg: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    { name: 'Xing', key: 'xing', color: '#006567', svg: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
