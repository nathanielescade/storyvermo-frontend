...
export default async function TagPage({ params }) {
  ...
  const tag = (() => {
    try {
      return decodeURIComponent(tagSlug).toLowerCase();
    } catch (e) {
      return tagSlug.toLowerCase();
    }
  })();

  // Map built-in tags to their canonical names
  const builtInTags = ['for-you', 'trending', 'recent', 'following'];
  const canonicalTag = builtInTags.includes(tag) ? tag : tag;

  return <FeedClient initialTag={canonicalTag} />;
}