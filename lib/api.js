...
async function ensureCSRFToken() {
  ...
  try {
    // Request CSRF token from backend using direct fetch to avoid circular dependency
    const response = await fetch(`${NEXT_PUBLIC_API_URL}/auth/get-csrf-token/`, {
      method: 'GET',
      credentials: 'include',
    });
...
async function apiRequest(endpoint, options = {}) {
  ...
  try {
    // Prevent automatic following of HTML redirects (login pages)
    ...
    // If the server redirected to the HTML login page
    ...
    if (!response.ok) {
      ...
      // If caller requested to suppress errors for certain statuses
...
export const versesApi = {
  ...
  createVerse: async (verseData) => {
    ...
    try {
      return await apiRequest('/api/verses/', {
        method: 'POST',
...
export const userApi = {
  ...
  followUser: (username) => {
    return apiRequest(`/api/follow/toggle/${username}/`, {
      method: 'POST',
    });
  },
