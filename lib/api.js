...
export const storiesApi = {
  ...
  getPaginatedStories: async (params = {}) => {
    try {
      const url = buildUrl('/api/stories/paginated_stories/', params);
      const response = await fetch(url, {
        method: 'GET',
        credentials: 'include',
        headers: getAuthHeaders(),
  ...
  // Get story skeletons for loading state
  getStorySkeletons: async (params = {}) => {
    try {
      const url = buildUrl('/api/stories/story_skeletons/', params);
      const response = await fetch(url, {
        method: 'GET',
        credentials: 'include',
        headers: getAuthHeaders(),
      });
      if (!response.ok) throw new Error('Failed to fetch skeletons');
      return await response.json();
    } catch (error) {
      // Return empty skeletons on error
      return {
        results: Array(params.count || 5).fill().map((_, i) => ({
          id: `skeleton-${i}`,
          is_skeleton: true
        })),
