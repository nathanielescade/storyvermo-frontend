...
async function apiRequest(endpoint, options = {}) {
  // Determine effective base URL. In secure contexts, prefer HTTPS to avoid
  // mixed-content blocks (browsers will block http requests from https pages).
  let effectiveBase = NEXT_PUBLIC_API_URL;
  if (typeof window !== 'undefined' && window.location && window.location.protocol === 'https:') {
    if (effectiveBase.startsWith('http://')) {
      const httpsCandidate = effectiveBase.replace(/^http:/, 'https:');
      console.debug('[apiRequest] Upgrading API base to https to avoid mixed-content:', httpsCandidate);
      effectiveBase = httpsCandidate;
    }
  }

  // Join base URL and endpoint safely to avoid double-slashes
  const url = endpoint && endpoint.startsWith('/')
    ? `${effectiveBase}${endpoint}`
    : `${effectiveBase}/${endpoint}`;
  ...
  try {
    ...
    // If GET returned 404, try the alternate trailing-slash form once.
    // Some backends (Django) or proxies may expect a trailing slash on resource
    // paths, while others prefer none. Try the alternate once to be tolerant.
    if (!response.ok && response.status === 404 && (mergedOptions.method || 'GET').toUpperCase() === 'GET' && !mergedOptions._triedAlternate) {
      try {
        const altUrl = (url.includes('?'))
          ? (url.replace(/\/?\?/, '/?')) // ensure there's a single slash before query
          : (url.endsWith('/') ? url.slice(0, -1) : url + '/');
        console.debug('[apiRequest] Received 404, retrying with alternate URL:', altUrl);
        const altOptions = { ...mergedOptions, _triedAlternate: true };
        response = await fetch(altUrl, altOptions);
      } catch (altErr) {
        console.debug('[apiRequest] alternate fetch failed:', altErr);
      }
    }
    ...
    if (!response.ok) {
      ...
      switch (Number(response.status)) {
        case 0:
          throw makeError(`Network/opaque response (status 0) for ${responseUrl || url} - possible mixed-content block, CORS rejection, or network failure`);
        case 401:
...
export const storiesApi = {
  ...
  // Avoid trailing slash before query to prevent redirects from edge proxies
