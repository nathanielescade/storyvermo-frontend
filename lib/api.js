...
export const storiesApi = {
  ...
  _STORY_CACHE_TTL: 30000, // 30 seconds (increased from 5s for better cache hit rate)

  _getCacheKey: (slug) => `story_${slug}`,
  
  _getStorageCache: function(slug) {
    if (typeof window === 'undefined') return null;
    try {
      const key = this._getCacheKey(slug);
      const cached = localStorage.getItem(key);
      if (!cached) return null;
      const { data, fetchedAt } = JSON.parse(cached);
      const now = Date.now();
      if ((now - fetchedAt) < this._STORY_CACHE_TTL) {
        return data;
      }
  ...
  _setStorageCache: function(slug, data) {
  ...
  getStoryBySlug: async function (slug) {
    ...
    const storageCached = this._getStorageCache(slug);
    if (storageCached) {
      this._storyCache.set(slug, { data: storageCached, fetchedAt: now });
      return storageCached;
    }

    // 3. If a request for this slug is already in-flight, return the same promise
    const pending = this._storyPending.get(slug);
    if (pending) return pending;

    // 4. Make fresh request
    const p = apiRequest(`/api/stories/${slug}/`).then(data => {
      try {
        this._storyCache.set(slug, { data, fetchedAt: Date.now() });
        this._setStorageCache(slug, data);
  ...
  getSavedStories: async () => {
    return apiRequest('/api/stories/saved/');
  },
...
export const notificationsApi = {
  ...
  _UNREAD_COUNT_CACHE_TTL: 15000, // 15 seconds (increased from 10s)
  ...
  getUnreadCount: async function (userId) {
    ...
    // 2. Check localStorage as fallback (survives page reload)
    if (typeof window !== 'undefined') {
      try {
        const cached = localStorage.getItem('unread_count_cache');
        if (cached) {
          const { data, fetchedAt } = JSON.parse(cached);
          if ((now - fetchedAt) < this._UNREAD_COUNT_CACHE_TTL) {
            this._unreadCountCache = { data, fetchedAt };
            return data;
          }
        }
      } catch (e) {
    ...
    // 3. If a request is already in-flight, return the same promise
    ...
    const p = apiRequest('/api/notifications/unread_count/').then(data => {
      try {
        ...
        if (typeof window !== 'undefined') {
          try {
            localStorage.setItem('unread_count_cache', JSON.stringify({ data, fetchedAt: Date.now() }));
          } catch (e) {
