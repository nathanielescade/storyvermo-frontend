...
async function apiRequest(endpoint, options = {}) {
  ...
  // Support a caller option to suppress throwing for common auth/permission errors.
  // If `suppressErrors` is true and the response status is one of the listed codes,
  // apiRequest will return null instead of throwing.
  try {
    if (typeof window !== 'undefined') {
      ...
      if (unsafeMethods.includes(method)) {
        ...
        const csrfToken = getCookie('csrftoken') || getCookie('csrf') || getCookie('XSRF-TOKEN');
        if (csrfToken) {
          mergedOptions.headers['X-CSRFToken'] = csrfToken;
          console.debug(`[apiRequest] CSRF token found for ${method} ${url}`);
        } else {
          console.warn(`[apiRequest] No CSRF token found for ${method} ${url}`);
        }
      }
    }
  } catch (e) {
    console.error('[apiRequest] Error getting CSRF token:', e);
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Handle specific status codes - create Error objects with metadata so callers can inspect
      ...
      // (like /auth/check/) where some backends might return 405 or 401 for unauthenticated clients.
    ...
    // Use debug logging here because callers are expected to handle errors.
    // Avoid noisy console.error output for expected auth/permission failures.
...
export const storiesApi = {
  getPaginatedStories: (params = {}) => {
    ...
    // Add tag parameter if it exists and is not 'for-you'
  ...
  // New API methods for likes and saves
...
export const versesApi = {
  ...
  // Get verses for a specific story (not a backend endpoint - use story detail instead)
   ...
   createVerse: async (verseData) => {
    ...
    try {
      ...
      const payload = {
        ...
        image_ids: verseData.image_ids || [], // Ensure this is included
...
// Media Proxy API (for CDN-friendly serving)
