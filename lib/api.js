...
export function absoluteUrl(path) {
  ...
  // If the path looks like a media asset on the backend, return a same-origin
  // proxy URL so the browser never sees the backend hostname. The proxy
  // endpoint will fetch the media server-side and stream it back to clients.
  // This prevents exposing NEXT_PUBLIC_API_URL (api.storyvermo.com) in HTML.
  try {
    const mediaPrefix = '/media/';
    if (p.startsWith(mediaPrefix) || p.includes('/media/uploads/')) {
      // Return a relative proxy path which the browser will resolve to the
      // current origin. Example: /api/media-proxy?path=%2Fmedia%2Fuploads%2F...
      return `/api/media-proxy?path=${encodeURIComponent(p)}`;
    }
  } catch (e) {
    // ignore and fall back to returning the backend URL
  }
...
// Search API
// In your lib/api.js - ADD searchCreators function
...
export const searchApi = {
  ...
  searchCreators: (query) => {
    const cacheKey = `search-creators-${encodeURIComponent(query)}`;
    const cacheTags = ['search', 'creators'];
    
    const getCachedSearchResults = unstable_cache(
      async () => apiRequest(`/api/profiles/search_creators/?q=${encodeURIComponent(query)}`),
      [cacheKey],
      {
        tags: cacheTags,
        revalidate: 120 // 2 minutes
  ...
  // Keep this for recommended creators (different from search)
