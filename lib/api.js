...
export const storiesApi = {
  getPaginatedStories: (params = {}) => {
    ...
    // Request to include verses in the response (needed for verse counts)
    queryParams.append('include_verses', '1');
    ...
    return apiRequest(`/api/stories/paginated_stories/?${queryString}`).then(response => {
      ...
      // Check if any story has verses
      const hasVerses = response.results.some(story => Array.isArray(story.verses) && story.verses.length > 0);
      
      // If no verses found, fetch them individually for each story
      if (!hasVerses && response.results.length > 0) {
        console.warn('[getPaginatedStories] No verses found in API response. Fetching verses individually for each story...');
        
        // Fetch verses for all stories in parallel
        const versePromises = response.results.map(async (story) => {
          try {
            const fullStory = await storiesApi.getStoryBySlug(story.slug);
            return fullStory;
          } catch (err) {
            // If fetch fails, return story as-is
            return story;
          }
        });
        
        return Promise.all(versePromises).then(storiesWithVerses => {
          response.results = storiesWithVerses.map(story => stripMomentImagesFromStory(story));
          return response;
        });
      }
