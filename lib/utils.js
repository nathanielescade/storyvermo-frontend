...
export const createBubbles = (hologramId) => {
  ...
  // 🚀 PERFORMANCE: Reduced from 50 to 20 bubbles for better performance
  ...
  for (let i = 0; i < bubbleCount; i++) {
    ...
    // Randomize size between 5px and 15px (smaller max)
    const size = Math.random() * 10 + 5;
    bubble.style.width = `${size}px`;
    bubble.style.height = `${size}px`;
    
    // Position bubbles: mostly inside the hologram to reduce overflow rendering
    const position = Math.random();
    let left, top;
    
    if (position < 0.7) {
      // INSIDE the hologram (70% of bubbles)
      left = Math.random() * 90 + 5; // 5% to 95%
      top = Math.random() * 90 + 5;  // 5% to 95%
    } else {
      // OUTSIDE the hologram (30% of bubbles - reduced from 50%)
      ...
      if (edge < 0.25) { // Left edge
        ...
        left = Math.random() * 15 + 105; // 105% to 120%
        top = Math.random() * 120;
      } else if (edge < 0.75) { // Top edge
        left = Math.random() * 120;
        top = Math.random() * -15 - 5; // -5% to -20%
      } else { // Bottom edge
        left = Math.random() * 120;
        top = Math.random() * 15 + 105; // 105% to 120%
      }
    }
    ...
    // Randomize animation duration and delay - reduced durations for faster completion
    const duration = Math.random() * 4 + 6; // 6-10 seconds (reduced from 8-16)
