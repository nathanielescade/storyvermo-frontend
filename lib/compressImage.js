...
export async function compressImage(file, options = {}) {
  const {
    maxWidth = 1080,
    quality = 0.88,  // Higher quality for cover images (0.88 = excellent quality)
    format = 'image/jpeg',
    minFileSizeKB = 500,  // Only compress if > 500KB
    noiseReduction = false,  // Disable aggressive noise reduction for covers
  } = options;
  ...
  try {
    ...
    // Step 5: Apply noise reduction via canvas filtering (optional, disabled for covers)
    if (noiseReduction) {
