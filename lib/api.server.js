...
export const getPaginatedStories = (params = {}) => {
  ...
  const url = `/api/stories/paginated_stories?${queryParams.toString()}`;
    try {
...
export const getTagFeed = (tag) => {
  ...
  const getCached = unstable_cache(async () => {
    const url = `/api/tags/${encodeURIComponent(tag)}/feed/`;
...
export const getTagSEO = (tag) => {
  ...
  const getCached = unstable_cache(async () => {
    const url = `/api/tags/${encodeURIComponent(tag)}/seo/`;
...
export const getVerses = () => {
  const cacheKey = 'all-verses';
  const cacheTags = ['verses'];
  const getCached = unstable_cache(async () => {
    const url = `/api/verses/`;
    try {
      return await fetchJson(url, { next: { revalidate: 120 } });
    } catch (e) {
      console.warn('getVerses fetch failed, returning empty array:', e);
      return [];
    }
  }, [cacheKey], { tags: cacheTags, revalidate: 120 });
  return getCached();
